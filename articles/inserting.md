...
If we don't count upserts, there are three insert functions in Monger:
`monger.collection/insert`, `monger.collection/insert-and-return`,
and `monger.collection/insert-batch`.

Let's first take a look at the former. It takes three arguments:
a database, collection name and document to insert:
...
your own data types that implement `java.util.List`:
...
Nested documents are inserted just the way one would expect: as nested
Clojure maps (or DBObjects):
...
Please make sure to read [MongoDB documentation on error handling of
batch
inserts](http://www.mongodb.org/display/DOCS/Inserting#Inserting-Bulkinserts)
...
standalone [Clojure validation library](http://clojurevalidation.info)
called Validateur. Validateur is inspired by Ruby's ActiveModel. It
has two central concepts: validators and validation sets.

Validateur is functional: validators are functions, validation sets
are higher-order functions, validation results are returned as values.
